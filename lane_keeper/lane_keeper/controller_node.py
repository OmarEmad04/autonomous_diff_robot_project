...
class LaneController(Node):
    ...
    def __init__(self):
        ...
        self.finish_line_detected = 0
        
        #Time variables for finish line handling
        self.lost_time_start = 0
        self.max_lost_duration = 2.0  # seconds
    ...
    def callback(self, msg):
        ...
        if msg.data == -1:
            if self.lost_time_start is None:
                self.lost_time_start = self.get_clock().now()
                self.get_logger().info("Line lost, starting timer...")
                
            elapsed_time = (self.get_clock().now() - self.lost_time_start).nanoseconds / 1e9
            
            if elapsed_time < self.max_lost_duration:
                cmd.linear.x = 1.0
                cmd.angular.z = 0.0
                self.pub.publish(cmd)
                return
            else:
                #Time exceeded, stop permanently
                self.is_permanently_stopped = True
                cmd.linear.x = 0.0
                cmd.angular.z = 0.0
                self.pub.publish(cmd)
                self.get_logger().info("Line lost for too long, stopping permanently.")
                return
        else:
        
            self.lost_time_start = None  # Reset timer
